...
void run() {
    scanf("%d%d", &h, &w);
    if(w <= h) swapped = false;
    else swapped = true, swap(w, h);
    ...
    REP(i, h * w) {
        ...
        for(map<string, int>::iterator j = best[i].begin(); j != best[i].end(); ++j) {
            int nrspace = 0;
            REP(k, w) if(isdigit(j->first[k])) ++nrspace;
            int nrcur = 0;
            REP(k, w) if(j->first[k] == j->first[col]) ++nrcur;
            //space
            if(nrspace > 0 || j->first[w] == 'N') {
                string next = j->first;
                int nval = j->second;
                if(next[col] == '+') next[col] = '*', ++nval;
                if(col - 1 >= 0 && next[col-1] == '+') next[col-1] = '*', ++nval;
            ...
            if(!isdigit(j->first[col]) || nrcur > 1 || nrspace == 1 && j->first[w] == 'Y') {
                ...
                if(isdigit(next[col]) || col - 1 >= 0 && isdigit(next[col-1])) next[col] = '*', ++nval;
                else next[col] = '+';
                norm(next);
                if(!best[i+1].count(next) || nval > best[i+1][next]) best[i+1][next] = nval, prev[i+1][next] = j->first;
            }
    ...
    for(map<string, int>::iterator j = best[h*w].begin(); j != best[h*w].end(); ++j) if(j->first[w] != 'N') {
            bool ok = true;
            REP(k, w) if(j->first[k] == '1') ok = false;
            if(!ok) continue;
    ...
    REP(i, h) REP(j, w) {
        if(isdigit(ret[i][j])) if((i == 0 || i == h - 1 || j == 0 || j == w - 1) && !haveentry) ret[i][j] = 'E', haveentry = true;
            else ret[i][j] = '.';
        else ret[i][j] = 'B';
    }
    ...
    if(swapped) {
        ...
        ret = vector<string>(w, string(h, '?'));
        REP(i, h) REP(j, w) ret[j][i] = tmp[i][j];
        swap(h, w);
    }
